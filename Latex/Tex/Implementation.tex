...
For example, \verb|native_set_pte()| is the function that acutally implements PTE operations. Other PTE operation functions such as \verb|set_pte()| and \verb|native_set_pte_atomic()| all invoke the \verb|native_set_pte()| function to accomplish their implementation. 
% implement their
...
In HyperPS, the HostOS has been deprived of the privileges on managing EPTs. This give HyperPS full control over the guest VM's virtual-physical memory mapping and protection. In our prototype, we hook all KVM's VMX and EPT functions to redirect EPT management to our HyperPS Space.
...
In the QEMU-KVM architecture, the KVM provdes a wrapper around these privileged instructions. The QEMU does not need to deal architecture specific details, it just need to invoke the wrapper functions in KVM with proper parameters.
% the KVM is responsible for executing these privileged instructions. The KVM provdes a wrapper around these privileged instructions. The QEMU does not need to deal architecture specific details, it just need to invoke KVM functions with proper parameters.
...
In particular, we prevent the HostOS/Hypervisor from loading a unverified EPTP. 
HyperPS interposes the execution of \verb|vmcs_writel()|, and the Context Management in the HyperPS Space Management component check the invoked parameters and verifies if the write to \verb|VM-Execution Control Fields.EPTP| is legal or not. 
HyperPS also forbids arbitrary write to the \verb|VM-Function Controls| field in \verb|VM-Execution Control Fields|.
The field \verb|VM-Function Controls| contains the physical address of the 4-KByte EPTP list. Invalid value in the EPTP list may lead to unverified EPTP value. 
...
HyperPS needs to hooks EPT functions that access to EPTP and EPT Paging Structures to HyperPS Space, and implement these services in the HyperPS Space.
In QEMU-KVM architecture, the KVM is the componet that maintainS EPTs while QEMU is responsible for requesting for memory allocation.
...
The KVM adopts the similar page table construction process to the kernel page table that fills EPT paging structures by handling EPT page fault. 
...
Normally, the KVM is not responsible for the allocation of physical pages, what KVM does is to synchronize the allocated physical page frame number (PFN) to the corresponding EPT paging-structure. We classify EPT operations in EPT Page Fault into Update. 
In virtualization environment, there is a memory reclamation technique, called Ballooning, that is used by the hypervisor to allow the physical host system to retrieve memories from certain guest virtual machine and share them with others. 
However, Ballooning may be abused to facilitate side channel attacks. 
In this paper, we take such situation into consideration. As depicted in Figure ref{fig:impept}, 
HyperPS will format the allocated physical memory frame before mapping it in the EPT during executing the function \verb|kvm_tdp_page_fault()|. More details are illustrated in Section . 
% TODO 添加章节的引用
...
Because all EPTs have been moved from the Normal Space to the HyperPS Space, though HyperPS does not interpose the traversal of EPT, HyperPS also need to instrument EPT traversal funtions, so that they can access EPTs. 
...
For example, the function \verb|shadow_walk_init()| is responsible for initializing the struct \verb|kvm_shadow_walk_iterator| structure which will record the vaule of EPTP and be used in the traversal of EPTs. We added the hook funtion inside the function \verb|shadow_walk_init()| to access EPT in HyperPS Space. 
...
As depected in Figure \ref{fig:impept}, we placed a hook before executing the funtion \verb|vmx_load_mmu_pgd()| inside this function. The function \verb|vmx_load_mmu_pgd()| is the final function that load the EPT. 
...
\verb|vcpu_enter_guest()| is an arch-specific function in KVM to start up a virtual machine. 
In this function, we placed a hook before executing the funtion \verb|vmx_load_mmu_pgd()| inside this function. 
...
\verb|kvm_shadow_walk_iterator| records the vaule of EPTP
\fi
...
KSM enables the KVM Hypervisor to examine two or more already running virtual machine and compare their memory. If any memory region or pages are identical, KSM reduces multiple identical memory pages to a single page. 
...
The KSM adopts two red-black tree: the unstable tree and the stable treee, to implement its service.
The unstable tree holds pointers to pages which have been found to be unchanged for a period of time. 
...
At runtime, for each page scanned, the KSM proceeds searching a match first in the stable tree that only contains already shared pages. If a match is found in the stable tree, KSM will merge this scanned page with the KSM page found in the stable. 
...
In this paper, as mentioned in Section \ref{ssub:ept_paging_structure_protection}, we also takes KSM into consideration. 
\verb|stable_tree_insert()|, \verb|stable_tree_append()| is the funtions in KSM to add the merged page into stable tree. 
