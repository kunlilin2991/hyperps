...
In a virtualization environment, the HostOS/hyperivor deprivileges the guest VM’s kernel and interposes all interactions between guest VMs and the physical memory. 
If the HostOS/hypervisor has been compromised, both isolation between VMs and virtual-physical mapping relationships for a VM will inevitable be tamperred. 
...
In the QEMU-KVM architecture, The KVM provdes a wrapper around privileged instructions and is respinsible for executing privileged instructions.
The QEMU does not need to deal architecture specific details, it just need to invoke KVM functions with proper parameters. 
HyperPS hooks KVM's VMCS and EPT functions into the HyperPS Space. In the HyperPS Space, Context Management in the HyperPS Space Management component check the invoked parameters and verifies if the inputed parameters is legal or not.  
...
Since VMCS is hidden in the HyperPS Space which is unaccessable by functions in the hypervisor space, all context management (accessing VMCS operations) will be trapped to HyperPS Space inevitablely.
...
In this paper, we propose a table: VM-Mark table to restrict hypervisor from loading a unverified EPTP value arbitrarily.
...
For example, different VMs with same kernel may share the same physical memory page frames that holds the kernel code. HyperPS takes this situation into consideration too. If one physical memory page frame is shared between VMs, HyperPS fills the field \verb|SharedID| with the one of the VM's \verb|OwnerID|.
