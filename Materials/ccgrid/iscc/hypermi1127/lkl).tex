...
The guest virtual machine is easy to be subveted once the hypervisor is compromised. 
...
Therefore, It is important to protect the guest virtual machine, especially the context and the memory of guest virtual machine from the compromised hypervisor. 

This paper proposes HyperMI, a novel approach to supply VM protection in running time based on a privilege-level secure execution environment in cloud computing against compromised hypervisor. 
Firstly, We propose a HyperMI world which is a secure and isolated trusted execution environment. The HyperMI world is designed to placed at the same privilege-level with the Hypervisor and does not rely on any addtional hardware or higher privileged levels than the Hypervisor. Secondly, we proposed the event-driven VM monitoring which would intercept the interaction between all vitual machines and the hypervisor and redirect them to our HyperMI world for VM proction. HyperMI decoupling the functions of interaction between the VM and the Hypervisor, and the funtions of address mapping of VM from the compromised hypervisor. Thus, crucial data structuures such as VMCS, EPT would be protected from been subverted. Thirdly, we implement an effective VM isolation to prevent attacker from compromising innocent VM through escape attack with a malicious VM initially. 
    % effective VM isolation and event-driven VM monitoring in order to prevent customers' sensitive data in VM from being leaked.
